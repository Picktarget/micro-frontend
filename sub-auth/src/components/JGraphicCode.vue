<template>
  <div class="gc-canvas" id="code-img" @click="reloadPic">
    <img :style="{ width: this.contentWidth + 'px', height: this.contentHeight + 'px' }" :src="imageUrl" />
    <!-- <canvas id="gc-canvas" :width="contentWidth" :height="contentHeight"></canvas> -->
  </div>
</template>

<script>
import { getAction, getBlob } from '@/api/http-method'
  ...
  methods: {
    ...
    randomCode() {
      ...
      return new Promise(resolve => {
        if (this.remote == true) {
          getBlob(`/code/image?width=${this.contentWidth}&height=${this.contentHeight}`, {
            headers: { imageDeviceId: deviceId }
          })
            .then(res => {
            ...
            .catch(() => {
              window.console.log('生成验证码连接服务器异常')
              this.code = 'BUG'
              resolve()
            })
        } else {
